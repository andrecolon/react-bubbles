...
- [k] Explain what a token is used for.
      >>Sets an item on the local client to authorize th viewing of compenents in the application
- [k] What steps can you take in your web apps to keep your data secure?
      >> Create protected pages that use tokens the can be accessed with the proper login credentials 
- [ ] Describe how web servers work. 
- [k] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers. 
      >> Not sure how to answer  CREATE = put READ = get UPDATE = post DELETE = delete 
...
- [k] Add PM as collaborator on Github.
- [ ] Clone your OWN version of Repo (Not Lambda's by mistake!).
- [k] Create a new Branch on the clone: git checkout -b `<firstName-lastName>`.
- [k] Implement the project on this Branch, committing changes regularly.
...
- [k] Submit a Pull-Request to merge <firstName-lastName> Branch into master (student's  Repo).
- [ ] Add your Project Manager as a Reviewer on the Pull-request
- [k] PM then will count the HW as done by  merging the branch back into master.
...
- [k] Construct an AXIOS request to retrieve a token from the server. You'll use this token to interact with the API
- [k] Save the token to localStorage
- [k] Build a `axiosWithAuth` module to create an instance of axios with the authentication header
- [k] Build a `PrivateRoute` component and use it to protect a route that renders the `BubblesPage` component

### Stage 2 - Consuming the API

- [k] When `BubblePages` renders, make a GET request to fetch the color data for your bubbles.
